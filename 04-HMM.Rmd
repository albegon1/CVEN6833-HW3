...
The code below fits HMM models of orders 2 through 6 and calculates the AIC  for each. Best order is the one with the least value of AIC.
...
We select the hmm model with the lowest AIC. In this case, this happens to be of order 2.

We rerun the HMM for best order (m=2). Below is the the model summary and the best state sequence resulting from it.
...
state1=ifelse(decoding==1,1,NA)
state2=ifelse(decoding==2,2,NA)
plot(flow$may-mean(flow$may)+1.5,type="l",ylab="",
     main="Modified May flows and states (supperposed)")
points(state1,col='blue')
points(state2,col='red')
...
Instead of simulating the flows from a "static" state sequence, the goal is to simulate the state as a prediction from a best logistic GLM. 
...
comb=leaps(X,Y, nbest=40,method="adjr2")$which
...
ENSO and PDO resulted to be insignificant covariates for the logistic regression. We tried to lag them with respect to the prediction year without success. 
...
for(isim in 1:nsim){
  ...
  for(j in 1:N){
    if(j==1){
      ...
      state[j]=ifelse(runif(1)>state.glm$fitted.values[j-1],1,2)
    }
    if(state[j]==1){
