...
x = mflow[,2:13] %>% `rownames<-`(mflow[,1]) %>% # Apr - Jun
  {rowSums(.[,4:6])} %>% {./10^6} # convert AF to MAF
```
...
state.1=ifelse(decoding==1,mean(x)-1,NA)
state.2=ifelse(decoding==2,mean(x)+1,NA)
plot(x,type="l",ylab="",
...
```{r}
xx = X[100:110,] # covariates 2000 - 2010 for glm
blind.flow = matrix(NA,nrow = nsim, ncol = 11) #init sim flows
for(isim in 1:nsim){
...
lines(x[101:111],col="red")
points(x[101:111],col="red")
```
...
plot(x[101:111],blind.median)
cor(x[101:111],blind.median)
```
